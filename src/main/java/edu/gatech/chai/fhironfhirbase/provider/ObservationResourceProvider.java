...
public class ObservationResourceProvider extends BaseResourceProvider {
    ...
    private void postConstruct() {
		getFhirbaseMapping().setFhirClass(Observation.class);
		getFhirbaseMapping().setTableName(ObservationResourceProvider.getType().toLowerCase());
	...
	public IBundleProvider findObservationsByParams(
		...
		if (theOrCodes != null) {
			String where = constructCodeWhereParameter(theOrCodes, fromStatement, "o.resource->'code'");
		...
		if (theDate != null) {
			String where = constructDateWhereParameter(theDate, fromStatement, "o", "effectiveDateTime");
			if (where != null && !where.isEmpty()) {
				whereParameters.add(where);
			}
		}
