...
public class ProcedureResourceProvider extends BaseResourceProvider {
    ...
    private void postConstruct() {
		getFhirbaseMapping().setFhirClass(Procedure.class);
		getFhirbaseMapping().setTableName(ProcedureResourceProvider.getType().toLowerCase());
	...
	public IBundleProvider findProceduresByParams(@OptionalParam(name = Procedure.SP_CODE) TokenOrListParam theOrCodes,
		...
		if (theOrCodes != null) {
			String where = constructCodeWhereParameter(theOrCodes, fromStatement, "proc.resource->'code'");
		...
		if (theDateParam != null) {
			String where = constructDateWhereParameter(theDateParam, fromStatement, "proc", "performedDateTime");
			if (where != null && !where.isEmpty()) {
				whereParameters.add(where);
			}
		}
